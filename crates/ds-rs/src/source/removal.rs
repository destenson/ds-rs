    ...
    fn stop_and_remove_source(&self, id: SourceId) -> Result<()> {
        ...
        match state_result {
            gst::StateChangeSuccess::Success => {
                println!(
                    "[{:.3}] Source {} state change to NULL: SUCCESS",
                    crate::timestamp(),
                    id
                );
            ...
            gst::StateChangeSuccess::Async => {
                println!(
                    "[{:.3}] Source {} state change to NULL: ASYNC",
                    crate::timestamp(),
                    id
                );
            ...
            gst::StateChangeSuccess::NoPreroll => {
                println!(
                    "[{:.3}] Source {} state change to NULL: NO PREROLL",
                    crate::timestamp(),
                    id
                );
                perform_source_cleanup(&pipeline, streammux, id, source)?;
        ...
        println!(
            "Successfully removed source {} - Remaining sources: {}",
            id,
            self.num_sources()?
        );
