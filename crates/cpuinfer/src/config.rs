            ...
            match current_section.as_str() {
                "property" => {
                    match key {
                        ...
                        "num-detected-classes" => config.num_detected_classes = value.parse().unwrap_or(80),
                        ...
                        "maintain-aspect-ratio" => config.maintain_aspect_ratio = value.parse().unwrap_or(1),
                        "symmetric-padding" => config.symmetric_padding = value.parse().unwrap_or(1),
                ...
                "class-attrs-all" => {
                    match key {
                        "pre-cluster-threshold" => config.pre_cluster_threshold = value.parse().unwrap_or(0.4),
                        "nms-iou-threshold" => config.nms_iou_threshold = value.parse().unwrap_or(0.5),
...
pub fn validate_config(config: &InferConfig) -> Result<(), String> {
    ...
    if config.batch_size == 0 || config.batch_size > 32 {
        return Err(format!("Invalid batch-size: {} (must be 1-32)", config.batch_size));
    }
    
    // Check process mode
    if config.process_mode != 1 && config.process_mode != 2 {
        return Err(format!("Invalid process-mode: {} (must be 1 or 2)", config.process_mode));
    }
    
    // Check thresholds
    if config.pre_cluster_threshold < 0.0 || config.pre_cluster_threshold > 1.0 {
        return Err(format!("Invalid pre-cluster-threshold: {} (must be 0.0-1.0)", 
            config.pre_cluster_threshold));
    }
    
    if config.nms_iou_threshold < 0.0 || config.nms_iou_threshold > 1.0 {
        return Err(format!("Invalid nms-iou-threshold: {} (must be 0.0-1.0)", 
