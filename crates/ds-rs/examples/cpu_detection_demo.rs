...
use ds_rs::backend::cpu_vision::{DetectorConfig, OnnxDetector, YoloVersion};
        ...
        Some(path) => {
            ...
            match test_real_detection(path) {
                ...
                Err(e) => {
                    ...
                    println!(
                        "This may be due to model format (float16 vs float32) or compatibility issues"
                    );
                    println!("\nTo fix this:");
                    println!("1. Run: python export_yolov5n_float32.py");
                    println!(
                        "2. Or install ultralytics and run: yolo export model=yolov5n.pt format=onnx half=False"
                    );
                }
            }
        }
