#![allow(unused)]
use crate::config::{AppConfig, Framerate, Resolution, VideoSourceConfig, VideoSourceType};
...
impl DefaultConfigValidator {
    ...
    fn validate_source_name(&self, name: &str) -> Result<()> {
        ...
        {
            return Err(SourceVideoError::config(
                "Source name can only contain alphanumeric characters, hyphens, and underscores"
                    .to_string(),
            ));
        }
    ...
    fn validate_rtsp_mount_point(
        ...
        if mount_point.is_empty() {
            return Err(SourceVideoError::config(
                "RTSP mount point cannot be empty".to_string(),
            ));
        }
    ...
    fn validate_source(&self, source: &VideoSourceConfig) -> Result<()> {
        ...
        match &source.source_type {
            ...
            VideoSourceType::Rtsp { port, .. } => {
                if *port == 0 {
                    return Err(SourceVideoError::config(
                        "RTSP port cannot be 0".to_string(),
                    ));
                }
            }
...
mod tests {
    ...
    fn test_source_name_validation() {
        ...
        assert!(
            validator
                .validate_source_name("source with spaces")
                .is_err()
        );
        assert!(
            validator
                .validate_source_name("a".repeat(100).as_str())
                .is_err()
        );
    }
}
