...
use crate::runtime::events::{ConfigurationEvent, EventBus};
use crate::source::{SourceState, VideoSource, create_source};
use crate::watch::{DirectoryWatcher, FileSystemEvent, WatcherManager};
...
impl VideoSourceManager {
    ...
    pub fn add_source(&self, config: VideoSourceConfig) -> Result<String> {
...

        {
            ...
            let mut name_map = self.name_to_id.write().map_err(|_| {
                SourceVideoError::resource("Failed to acquire write lock on name map")
            })?;
    ...
    pub fn update_source(&self, id_or_name: &str, config: VideoSourceConfig) -> Result<()> {
        ...
        let current_state = {
            let sources = self.sources.read().map_err(|_| {
                SourceVideoError::resource("Failed to acquire read lock on sources")
            })?;
        ...
        F: FnOnce(&mut VideoSourceConfig) -> Result<()>,
    {
        ...
        Err(SourceVideoError::config(
            "In-place modification not yet supported",
        ))
    }
    ...
    fn resolve_id(&self, id_or_name: &str) -> Result<String> {
        if Uuid::parse_str(id_or_name).is_ok() {
            ...
            let name_map = self.name_to_id.read().map_err(|_| {
                SourceVideoError::resource("Failed to acquire read lock on name map")
            })?;
    ...
    pub fn add_file_list(&self, config: FileListConfig) -> Result<Vec<String>> {
        ...
        for (index, file_path) in config.files.iter().enumerate() {
            let container =
                crate::file_utils::detect_container_format(std::path::Path::new(file_path))
    ...
    pub async fn add_watched_directory<P: AsRef<Path>>(
        ...
        {
            let mut path_map = self.path_to_source.write().map_err(|_| {
                SourceVideoError::resource("Failed to acquire write lock on path map")
            })?;
        ...
        if let Some(ref mut watcher_manager) = self.watcher_manager {
            let watcher_id = watcher_manager
                .add_directory_watcher(path, recursive)
                .await?;
            log::info!(
                "Added directory watcher: {} for path: {}",
                watcher_id,
                path.display()
            );
    ...
    pub async fn handle_file_event(&mut self, event: FileSystemEvent) -> Result<()> {
        ...
        // Emit event
        self.event_bus
            .emit(ConfigurationEvent::FileSystemChange {
