...
use ds_rs::{BackendManager, BackendType, PlatformInfo, init};
use std::sync::Arc;
...
fn test_backend_manager_auto_detection() {
    ...
    println!(
        "  - Supports inference: {}",
        capabilities.supports_inference
    );
...
fn test_backend_element_mapping() {
    ...
    assert_eq!(
        backend.get_element_mapping("nvvideoconvert"),
        Some("identity")
    );
    assert_eq!(
        backend.get_element_mapping("nveglglessink"),
        Some("fakesink")
    );
}
