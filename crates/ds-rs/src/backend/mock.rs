...
impl MockBackend {
    ...
    fn create_mock_bin(name: &str, internal_elements: usize) -> Result<gst::Element> {
        ...
        if !elements.is_empty() {
            let sink_pad = elements[0].static_pad("sink").ok_or_else(|| {
                DeepStreamError::PadLinking(format!("Failed to get sink pad from {}", name))
            })?;
            let src_pad = elements[elements.len() - 1]
                .static_pad("src")
                .ok_or_else(|| {
                    DeepStreamError::PadLinking(format!("Failed to get src pad from {}", name))
                })?;
...
impl Backend for MockBackend {
    ...
    fn create_inference(&self, name: Option<&str>, config_path: &str) -> Result<gst::Element> {
        log::debug!(
            "Mock backend: Creating mock inference with config: {}",
            config_path
        );
