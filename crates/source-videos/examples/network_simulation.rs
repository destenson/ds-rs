...
use source_videos::network::{
    NetworkConditions, NetworkController, NetworkProfile, NetworkSimulator, StandardProfiles,
};
...
fn demo_network_profiles() {
    ...
    println!(
        "  Error Recovery: {:?}",
        StandardProfiles::for_error_recovery()
    );
...
fn demo_connection_drops() {
    ...
    println!(
        "Connection restored: {}",
        !simulator.is_connection_dropped()
    );
