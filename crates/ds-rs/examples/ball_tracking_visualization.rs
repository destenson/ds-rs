...
use ds_rs::{
    BackendManager, BackendType, DeepStreamError, MetadataBridge, PipelineBuilder, Result,
    SourceController, SourceEvent, SourceId, init, timestamp,
};
...
impl BallTrackingApp {
    ...
    fn new(args: &Args) -> Result<Self> {
        ...
        if backend_manager.backend_type() == BackendType::DeepStream {
            builder = builder.set_property_from_str(
                "detector",
                "config-file-path",
                "models/ball_detection_config.txt",
            );
        }
        ...
        let streammux = pipeline_arc.get_by_name("streammux").ok_or_else(|| {
            DeepStreamError::ElementNotFound {
                element: "streammux".to_string(),
            }
        })?;

        // Create source controller for dynamic source management
        let source_controller = Arc::new(SourceController::new(pipeline_arc.clone(), streammux));
        ...
        event_handler.register_callback(move |event| match event {
            SourceEvent::SourceAdded { id, uri } => {
                log::info!(
                    "[{:.3}] Source {:?} added to pipeline (URI: {})",
                    timestamp(),
                    id,
                    uri
                );
            ...
            SourceEvent::StateChanged {
                ...
                log::debug!(
                    "[{:.3}] Source {:?} state changed from {:?} to {:?}",
                    timestamp(),
                    id,
                    old_state,
                    new_state
    ...
    fn start(&self) -> Result<()> {
        log::info!(
            "[{:.3}] Starting ball tracking visualization pipeline",
            timestamp()
        );
        self.pipeline.set_state(gst::State::Playing).map_err(|e| {
            DeepStreamError::Pipeline(format!("Failed to set pipeline to playing: {:?}", e))
        })?;
    ...
    fn stop(&self) -> Result<()> {
        ...
        self.pipeline.set_state(gst::State::Null).map_err(|e| {
            DeepStreamError::Pipeline(format!("Failed to set pipeline to null: {:?}", e))
        })?;
        Ok(())
    }
        ...
        while self.running.load(Ordering::SeqCst) {
                        ...
                        if let Some(structure) = element_msg.structure() {
                                ...
                                || structure.name() == "application/x-gst-navigation"
                            {
                                ...
                                if let Ok(event_type) = structure.get::<String>("event") {
                                    ...
                                    {
                                        log::info!(
                                            "[{:.3}] Window closed, stopping pipeline",
                                            timestamp()
                                        );
                                        self.running.store(false, Ordering::SeqCst);
                                        break;
                    ...
                    MessageView::Application(app_msg) => {
                        if let Some(structure) = app_msg.structure() {
                            if structure.name() == "window-closed" {
                                log::info!(
                                    "[{:.3}] Window closed (application message), stopping pipeline",
                                    timestamp()
                                );
                                self.running.store(false, Ordering::SeqCst);
                                break;
            ...
            if last_stats_print.elapsed() >= Duration::from_secs(1) {
                ...
                if state != gst::State::Playing && self.running.load(Ordering::SeqCst) {
                    log::warn!(
                        "[{:.3}] Pipeline not in PLAYING state: {:?}",
                        timestamp(),
                        state
                    );
                }
...
fn main() -> Result<()> {
    ...
    let primary_source = match args.source_type {
        SourceType::File => {
            if let Some(input) = args.input {
                ...
                if video_path.exists() {
                    format!(
                        "file:///{}",
                        video_path.display().to_string().replace("\\", "/")
                    )
                } else {
                    log::warn!(
                        "[{:.3}] Default test video not found, using test pattern",
                        timestamp()
                    );
        ...
        SourceType::Rtsp => args.input.unwrap_or_else(|| {
            log::info!(
                "[{:.3}] No RTSP URL provided, using default rtsp://127.0.0.1:8554/test1",
                timestamp()
            );
            "rtsp://127.0.0.1:8554/test1".to_string()
        }),
        SourceType::Test => "videotestsrc://".to_string(),
        SourceType::Camera => {
    ...
    log::info!(
        "[{:.3}] Ball tracking visualization example completed",
        timestamp()
    );
    Ok(())
}
