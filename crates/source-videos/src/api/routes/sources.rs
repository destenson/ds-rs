...
pub async fn list_sources(
    ...
    let responses: Vec<SourceResponse> = sources.into_iter().map(SourceResponse::from).collect();
        ...
        let result = match operation.operation {
            crate::api::models::BatchOperationType::Add => {
                ...
                let source_type = match operation.source.source_type {
                    ...
                    SourceTypeRequest::File { path, container } => {
                        let container =
                            container.unwrap_or(crate::config_types::FileContainer::Mp4);
                        VideoSourceType::File { path, container }
                    }
                    SourceTypeRequest::Rtsp { mount_point, port } => VideoSourceType::Rtsp {
                        mount_point,
                ...
                let config = VideoSourceConfig {
                    ...
                    source_type,
                    resolution: operation.source.resolution.unwrap_or(
                        crate::config_types::Resolution {
                            width: 1920,
                            height: 1080,
                        },
                    ),
                    framerate: operation.source.framerate.unwrap_or(
                        crate::config_types::Framerate {
                            numerator: 30,
                            denominator: 1,
                        },
                    ),
