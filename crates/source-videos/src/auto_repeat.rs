...
use crate::source::{SourceState, VideoSource};
...
impl GaplessLooper {
    ...
    pub fn apply_to_pipeline(&self, pipeline: &gst::Pipeline) -> Result<()> {
        let stop_time = self
            .segment_stop
            .unwrap_or(gst::ClockTime::from_seconds(u64::MAX));
...
impl AutoRepeatManager {
    ...
    pub fn add_source(&mut self, source: Box<dyn VideoSource>) {
        let looping_source =
            LoopingVideoSource::new(source).with_config(self.global_config.clone());
...
pub fn enable_auto_repeat_for_source(
    ...
    if let Some(pipeline) = source.get_pipeline() {
        ...
        let bus = pipeline
            .bus()
            .ok_or_else(|| SourceVideoError::pipeline("No bus available for auto-repeat setup"))?;
...
mod tests {
    ...
    fn test_looping_video_source_creation() {
        let mock_source = Box::new(MockVideoSource::new(
            "test-id".to_string(),
    ...
    fn test_loop_config() {
        ...
        assert_eq!(
            looping_source.loop_config.gap_duration,
