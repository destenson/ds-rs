...
use source_videos::{
    Result, RtspServerBuilder, VideoSourceConfig, VideoSourceType,
    config_types::{FileContainer, Framerate, Resolution, VideoFormat},
    detect_container_format, init,
};
...
async fn main() -> Result<()> {
    ...
    if args.len() < 2 {
        ...
        eprintln!(
            "Example: {} video1.mp4 video2.avi /path/to/video3.mkv",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    if !sources.is_empty() {
        ...
        println!("========================================");
        println!(
            "A mount point is the path part of an RTSP URL that identifies a specific video stream."
        );
        println!(
            "For example, in 'rtsp://localhost:8554/file1_video', the mount point is 'file1_video'."
        );
        println!(
            "Each video file gets its own unique mount point so you can access it independently."
        );
        println!();
