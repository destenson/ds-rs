...
fn copy_onnx_dlls() {
    ...
    for dll_name in &dll_files {
        ...
        if source.exists() {
            ...
            println!(
                "cargo:warning=ONNX Runtime DLL not found: {}. This may cause runtime errors with ort feature.",
                source.display()
            );
            println!(
                "cargo:warning=The DLLs should be automatically downloaded by the ort crate build process."
            );
        }
    }
}
