        ...
        match source_type {
            "pattern" => {
                ...
                match sv.add_test_pattern(&mount_name, pattern) {
                    Ok(id) => output.print_success(&format!(
                        "Added pattern '{}' with ID: {} ({})",
                        pattern, id, mount_name
                    )),
            ...
            "directory" => {
                ...
                output.print_info(&format!(
                    "Adding directory: {} (recursive: {}, watch: {})",
                    path.display(),
                    recursive,
                    watch
                ));
            ...
            _ => {
                output.print_error(&format!(
                    "Unknown source type: {}. Use 'pattern', 'directory', or 'file'",
                    source_type
                ));
            }
        }
...
impl ReplCommand for StatusCommand {
    async fn execute(
        ...
        let playing_count = sources
            .iter()
            .filter(|s| s.state.to_string() == "PLAYING")
            .count();
    ...
    ($name:ident, $cmd_name:literal, $desc:literal, $usage:literal) => {
        ...
        impl ReplCommand for $name {
            async fn execute(
                ...
                output.print_info(&format!(
                    "'{}' command - placeholder implementation",
                    $cmd_name
                ));
                Ok(CommandResult::Continue)
            }
