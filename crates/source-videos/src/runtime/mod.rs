...
use differ::{ConfigChange, ConfigDiffer};
...
impl RuntimeManager {
        ...
        match applicator.apply_changes(changes.clone()).await {
            Ok(()) => {
                ...
                *current = new_config.clone();
                ...
                // Emit success event
                self.event_bus
                    .emit(ConfigurationEvent::ConfigApplied {
                        changes: changes.len(),
                    })
                    .await;
            ...
            Err(e) => {
                ...
                // Emit failure event
                self.event_bus
                    .emit(ConfigurationEvent::ConfigFailed {
                        error: e.to_string(),
                    })
                    .await;
    ...
    async fn rollback_to(&self, config: AppConfig) -> Result<()> {
        ...
        *current = config;
        ...
        self.event_bus
            .emit(ConfigurationEvent::ConfigRolledBack)
            .await;
    ...
    pub async fn update_source(&self, source_name: &str, config: VideoSourceConfig) -> Result<()> {
        ...
        let source_index = current
            ...
            .ok_or_else(|| {
                SourceVideoError::config(format!("Source '{}' not found", source_name))
            })?;
        ...
        self.event_bus
            .emit(ConfigurationEvent::SourceUpdated {
    ...
    pub async fn add_source(&self, config: VideoSourceConfig) -> Result<String> {
        ...
        self.event_bus
            .emit(ConfigurationEvent::SourceAdded {
    ...
    pub async fn remove_source(&self, source_name: &str) -> Result<()> {
        ...
        self.event_bus
            .emit(ConfigurationEvent::SourceRemoved {
                source: source_name.to_string(),
            })
            .await;
